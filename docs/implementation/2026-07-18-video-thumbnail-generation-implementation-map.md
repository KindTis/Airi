...
| IMP-016 | 필수 | 생성 중 충돌 동작 차단 | Task 4 Step 7 | 생성부터 임시 정리 종료까지 파일 선택·생성·초기화·141jav 적용·저장만 비활성화하고 handler guard를 둔다. F1 취소·X·Escape는 계속 가능하며 창이 열려 있으면 종료 후 재활성화한다. | `MetadataEditorWindow.xaml(.cs)`, 수명주기 테스트 | 구현 | 다섯 버튼 상태·Cancel 활성·guard·재활성화 테스트 | 통과 | 미판정 | |
...
| IMP-021 | 필수 | F1 수명주기 회귀 | Task 4 Step 3~5·9 | 계획서가 명명한 `MetadataEditorWindowThumbnailGenerationTests` 전체와 기존 ViewModel 테스트가 통과한다. | `tests/Airi.Tests/MetadataEditorWindowThumbnailGenerationTests.cs` | 구현 | 계획서의 Task 4 필터 명령 실행 | 통과 | 미판정 | |
| IMP-022 | 필수 | 전체 자동 회귀·빌드·정적 검사 | Task 5 Step 1·2·4 | 신규 범위 테스트, 전체 테스트, Debug 빌드, placeholder 검색, `git diff --check`가 통과하고 새 경고·설명되지 않은 변경이 없다. | 전체 변경 및 테스트 | 구현 | 신규 범위 48개, 전체 331개, Debug 빌드, placeholder·diff 검사 | 통과 | 미판정 | |
| IMP-023 | 필수 | 지식 그래프 갱신 | Task 5 Step 3 | 원본 checkout의 기존 graph·manifest 기준점을 worktree의 무시된 `graphify-out`에 복제한 상태에서, 코드 변경 뒤 worktree에서 `graphify update .`를 실행해 새 타입 관계를 기존 graph에 증분 반영한다. 원본 checkout의 graph는 갱신하지 않는다. | worktree `graphify-out/*`(Git 무시 산출물) | 구현 | 기준 SHA-256 일치, 최종 `graphify update .` 성공, 2,498 nodes/4,780 links와 새 converter 관계 확인 | 통과 | 미판정 | |
| IMP-024 | 필수 | 수동 UI 스모크 | Task 5 Step 5 | 앱 실행에서 버튼 순서, 포인터·키보드 선택/다시 생성/취소, 원자적 후보 교체, 생성 중 F1 종료, 최종 캐시 보존·회수를 계획 체크리스트대로 확인한다. | 실행 중 WPF UI | 구현 | 실제 H.264로 `dotnet run --project Airi.csproj --no-build`, Windows UI Automation·포인터·키보드와 대응 자동 테스트를 함께 기록 | 통과 | 미판정 | |
...
- 이번 루프가 생성하거나 수정한 경로: `docs/implementation/2026-07-18-video-thumbnail-generation-implementation-map.md`
- 이번 루프 중 생성한 커밋:
...
- 신규 범위 최종 검증: 계획 필터 명령 — 48 통과, 0 실패.
- 전체 품질 게이트: restore 성공, Debug build 경고 0·오류 0, 전체 테스트 331 통과·0 실패.
- 정적 검사: 필수 변경 파일 placeholder 0개, `git diff --check b8388cf..HEAD` exit code 0, worktree `git status --short` 출력 없음.
- graphify 최종 갱신: `graphify update .` exit code 0, 2,498 nodes/4,780 links, 최종 `graph.json` SHA-256 `812588F6FC5460BEC927AE7865FE1A88D1FB1528C4DF09FFBC531867DCF18B3C`. `OnLoadImageSourceConverter`와 `MetadataEditorWindow` 관계를 `explain`/`path`로 확인했다. `hooks.json` zero-node 경고 1건은 기존 비코드 입력에 대한 비차단 경고다.
...
- 수동 UI 스모크 후속: 선택 완료 시 세션 루트 0·FFmpeg 0, 수정 뒤 F1 취소 시 정리 경고 없음·생성 cache 0·세션 루트 0·FFmpeg 0을 확인했다. 다시 생성 실패 시 기존 후보 유지, 생성/선택 중 Cancel·X·Escape, 저장 캐시 행렬, 주입된 정리 실패 절대 경로 경고는 대응 WPF 자동 테스트로 보완했다.
